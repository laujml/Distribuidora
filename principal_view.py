...
class VentanaPrincipalProveedor(QWidget):
    ...
    def initUI(self):
        ...
        botones = [
            ("Agregar proveedor", lambda: self.stacked_widget.setCurrentIndex(1)),
            ("Eliminar proveedor", lambda: self.stacked_widget.setCurrentIndex(2)),
            ("Actualizar proveedor", lambda: self.stacked_widget.setCurrentIndex(3)),
            ("Buscar proveedor", lambda: self.stacked_widget.setCurrentIndex(4))
        ]
        ...
        for i, (texto, funcion) in enumerate(botones):
            ...
            btn.setStyleSheet(
                "background-color: white; color: black; border-radius: 12px; font-size: 15px; "
                "font-weight: 600; padding-top: 90px; padding-bottom: 10px;"
            )
            if i < 2:
