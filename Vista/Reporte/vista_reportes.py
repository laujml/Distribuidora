...
class ReportesView(QWidget):
    def __init__(self):
        ...
        # Obtener tamaño de pantalla para hacer el layout responsivo
        screen = QApplication.primaryScreen()
        screen_size = screen.size()
        self.screen_width = screen_size.width()
        self.screen_height = screen_size.height()
        
        # Definir tamaño mínimo de la ventana
        min_width = min(1000, int(self.screen_width * 0.8))
        min_height = min(800, int(self.screen_height * 0.8))
        self.setMinimumSize(min_width, min_height)
        
        # Permitir que la ventana se expanda
    ...
    def create_period_combo(self):
        # ComboBox para seleccionar el periodo
        self.period_cb = QComboBox()
        self.period_cb.addItems(["Semanal", "Mensual"])
        self.period_cb.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        Styles.apply_styles(self.period_cb)
    ...
    def create_summary_section(self, layout):
        # Sección de resumen de totales y top
    ...
    def create_charts_section(self, layout):
        # Sección de gráficos con tamaño adaptable
        ...
        # Configurar altura más grande para los gráficos
    ...
    def create_buttons_section(self, layout):
        # Sección de botones de exportar y actualizar
    ...
    def resizeEvent(self, event):
        # Evento para redimensionar la ventana y ajustar fuentes
    ...
    def populate_productos(self, productos):
        # Llena el ComboBox de productos con los datos recibidos
    ...
    def update_summary(self, total_ventas, total_pedidos, top_producto, top_cliente):
        # Actualiza los labels del resumen con los valores calculados
        # Se verifica que los valores sean correctos y se formatean
        try:
            total_ventas = float(total_ventas) if total_ventas is not None else 0.0
        except Exception:
            total_ventas = 0.0
        try:
            total_pedidos = int(total_pedidos) if total_pedidos is not None else 0
        except Exception:
            total_pedidos = 0
        top_producto = top_producto if top_producto else "-"
        top_cliente = top_cliente if top_cliente else "-"
    ...
    def actualizar_tabla(self, datos):
        ...
        for row, (fecha, cliente, producto, cantidad, subtotal) in enumerate(datos):
            ...
            # Verificar que cantidad sea entero y subtotal flotante
            try:
    ...
    def update_graphs(self, sales_data, client_data, best_products, worst_products):
        ...
        if sales_data:
            try:
                ...
                totals = [float(t) if t is not None else 0.0 for t in totals]
                self.axes[0, 0].plot(dates, totals, color='#969fa3', linewidth=2)
            except Exception:
                self.axes[0, 0].plot([], [], color='#969fa3', linewidth=2)
        ...
        if client_data:
            try:
                ...
                client_totals = [float(t) if t is not None else 0.0 for t in client_totals]
                self.axes[0, 1].bar(clients, client_totals, color='#969a9a')
            except Exception:
                self.axes[0, 1].bar([], [], color='#969a9a')
        ...
        if best_products:
            try:
                ...
                quantities = [int(q) if q is not None else 0 for q in quantities]
                self.axes[1, 0].barh(products, quantities, color='#969a9a')
            except Exception:
                self.axes[1, 0].barh([], [], color='#969a9a')
        ...
        if worst_products:
            try:
                ...
                self.axes[1, 1].barh([], [], color='#969a9a')
    ...
    def set_start_date(self, start_date):
        # Establece la fecha de inicio en el filtro
